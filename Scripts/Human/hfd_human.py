...
for patient in patient_list:
    ...
    hfd_chan_E1 = [compute_higuchi_fd(np.expand_dims(epochs[idx][0], axis = 0)) for idx in all_indices]
    hfd_chan_E2 = [compute_higuchi_fd(np.expand_dims(epochs[idx][1], axis = 0)) for idx in all_indices]
    hfd_chan_F3 = [compute_higuchi_fd(np.expand_dims(epochs[idx][2], axis = 0)) for idx in all_indices]
    hfd_chan_C3 = [compute_higuchi_fd(np.expand_dims(epochs[idx][3], axis = 0)) for idx in all_indices]
    hfd_chan_O1 = [compute_higuchi_fd(np.expand_dims(epochs[idx][4], axis = 0)) for idx in all_indices]
    hfd_chan_M2 = [compute_higuchi_fd(np.expand_dims(epochs[idx][5], axis = 0)) for idx in all_indices]
